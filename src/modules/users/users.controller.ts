...
export class UsersController {
  ...
  async getAllUsers(@Body('jData') data,@Res() res:Response) {
    try {
      ...
      res.status(HttpStatus.OK).send({ data: paginatedData.data, meta: paginatedData.meta });
  ...
  async getUserProfile(@GetUser() user: { userId: number }): Promise<UserEntity> {
  ...
  async verifyEmail(@Param('token') token, @Res() res: Response) {
    try {
      var isEmailVerified = this.userService.verifyEmail(token);
  ...
  async sendEmailVerification(@Param() params, @Res() res: Response) {
    try {
      await this.userService.createEmailToken(params.email);
      
      var isEmailSent = await this.emailTokenSend.sendEmailVerification(params.email);
      if (isEmailSent) {
  ...
  async sendPasswordResetToken(@Param() params, @Res() res: Response) {
    try {
      let isEmailSent = await this.userService.sendResetPasswordVerification(
        params.email,
      );
  ...
  async verfiyResetPasswordToken(@Query() param, @Res() res: Response) {
    try {
      var isEmailVerified = await this.userService.verifyResetPasswordToken(
        param.token,
        param.email,
      );
      if (isEmailVerified)
  ...
  async updateUser(@Body() user: UpdateProfileDto, @Param('id') id: string,@Res() res:Response) {
    try {
      ...
      if (error instanceof InValidDataError) {
        res.status(HttpStatus.BAD_REQUEST).send(inValidDataRes([error.message]));
      }
  ...
  @Post(':id/updateRole')
  @UsePipes(ValidationPipe)
  async updateRole(@Body() user: UpdateRole, @Param('id') id: string,@Res() res:Response) {
    try {
  ...
  async updatePwd(
    ...
    try {
      const user = await this.userService.repository.findOne({where:{email:data.email}});
      if (!user) {
